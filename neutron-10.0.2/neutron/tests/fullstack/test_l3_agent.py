...
class TestHAL3Agent(TestL3Agent):
    ...
    def _get_state_file_for_main_agent(self, router_id):
        for host in self.environment.hosts:
            keepalived_state_file = os.path.join(
                host.neutron_config.state_path, "ha_confs", router_id, "state")

            if self._get_keepalived_state(keepalived_state_file) == "main":
                return keepalived_state_file

    def test_keepalived_multiple_sighups_does_not_forfeit_mainship(self):
        ...
        common_utils.wait_until_true(
            functools.partial(
                ...
                router['id']),
            timeout=90)
        keepalived_state_file = self._get_state_file_for_main_agent(
            router['id'])
        ...
        # main. This will indicate that the Throttler works. We want to check
